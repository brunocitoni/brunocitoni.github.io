...
function findTeams(playingPlayers) {
    ...
    for (i = 0; i < c.length / 2; i++) {
        whiteTeamLocal.push(c[i]);
        blackTeamLocal.push(c[c.length - i - 1]);
    }
    ...
    for (i = 0; i < whiteTeamLocal.length; i++) {
        for (j = 0; j < whiteTeamLocal[i].length; j++) {
            increment = playerValues[whiteTeamLocal[i][j] - 1];
            summation = summation + increment;
        }

        // only save in new array if value of combination is good
        if (summation == desiredTeamValue) {
            whiteTeamNew.push(whiteTeamLocal[i]);
            blackTeamNew.push(blackTeamLocal[i]);
        }
    ...
    if ((Array.isArray(whiteTeamNew) && whiteTeamNew.length)) {
        ...
        whiteTeamLocal = whiteTeamNew;
        blackTeamLocal = blackTeamNew;
        
        // workaround to save length for random calculation 
        len = whiteTeamLocal.length;
        
        out = { whiteTeamLocal, blackTeamLocal };
...
function displayTeam() {
    ...
    // pick random value for team excluding the current one
    // if you only have 1 possible combination pick it with not exclusions
    // TODO instead of this you can just not allow the refresh team buttons to be pressed!
    if (whiteTeam.length != 1)
        var strUser = getRandomWithOneExclusion(whiteTeam.length,oldRandom);
    else 
        var strUser = 0;
    
    oldRandom = strUser; // save current team displayed in memory
    
    // TODO display which team is selected somehow (also to check all possible teams get actuall displayed)
    document.getElementById("teamSelected").innerHTML = oldRandom;
    ...
    for (j = 0; j < whiteTeam[strUser].length; j++) {
        ...
        } else if (j < whiteTeam[strUser].length) { //format last player differently
            string1 = string1 + playingPlayers.names[whiteTeam[strUser][j] - 1];
            string2 = string2 + playingPlayers.names[blackTeam[strUser][j] - 1];
        }
    }
...
function onClickFindTeamButton() {
    ...
    if (!isEven(playingPlayers.names.length) || playingPlayers.names.length == 0) {
        ...
        whiteTeam = combinationOutput.whiteTeamLocal;
        blackTeam = combinationOutput.blackTeamLocal;
